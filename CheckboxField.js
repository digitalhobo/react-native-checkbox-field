		...
		return (
			<TouchableOpacity onPress={onSelect} disabled={disabled}>
				<View style={containerStyle}>
					{labelSide === 'left' ? <View style={{ flex: 0.9 }}><Text style={labelStyle}>{label}</Text></View> : null}
					<Checkbox
						selected={selected}
						disabled={disabled}
						onSelect={onSelect}
					...
					{labelSide === 'right' ? <View style={{ flex: 0.9 }}><Text style={[labelStyle]}>{label}</Text></View> : null}
				</View>
			</TouchableOpacity>
		);
	}
